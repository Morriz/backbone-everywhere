    ...
    request.on('response', function(response) {
      ...
      response.on('end', function() {
        ...
        if(rpcRequest.hasOwnProperty('result')) {
          if (callback)
            callback(null, rpcRequest.result);
        }
        else {
          if (errback)
            errback(rpcRequest.error);
        }
      });
    });
  };
...
Server.prototype.handlePOST = function(req, res) {
  var buffer = '';
  var self = this;
  var handle = function (rpcRequest) {
    
    // Check for the required fields, and if they aren't there, then
    // dispatch to the handleInvalidRequest function.
    if(!(rpcRequest.method && rpcRequest.params && rpcRequest.id)) {
      return Server.handleInvalidRequest(req, res);
    }

    if(!self.functions.hasOwnProperty(rpcRequest.method)) {
      return Server.handleInvalidRequest(req, res);
    }
